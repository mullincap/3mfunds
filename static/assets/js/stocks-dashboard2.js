...
/* Total Investments Chart (REAL DATA) */
var element = document.getElementById("totalInvestmentsStats");
if (element !== null) {

    fetch("/api/investments/timeseries")
        .then(response => response.json())
        .then(data => {

            // Build series in Apex format: [timestamp_ms, value]
            const portfolioSeries = data.timestamps.map((ts, i) => {
                return [new Date(ts).getTime(), data.portfolio_value[i]];
            });

            const investedSeries = data.timestamps.map((ts, i) => {
                return [new Date(ts).getTime(), data.invested_value[i]];
            });

            const returnsSeries = data.timestamps.map((ts, i) => {
                return [new Date(ts).getTime(), data.total_returns[i]];
            });
            ...
            var options = {
                ...
                colors: ["#845adf", "#23b7e5", "#4ecc48"], // three series colors
                stroke: {
                    width: [1.2],
                    curve: ['smooth']
                },
            ...
            chart2 = new ApexCharts(document.querySelector("#totalInvestmentsStats"), options);
            chart2.render();
        });
}
